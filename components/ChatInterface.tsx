...
export function ChatInterface() {
  ...
  // State to control custom scrollbar visibility
    ...
    <>
      <div className="chat-interface flex flex-col h-full rounded-xl border border-border/40 overflow-hidden shadow-lg bg-card/50 backdrop-blur-sm">
        ...
        <div className="border-b border-border/40 p-4 flex items-center justify-between bg-card/80">
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={() => setShowCustomScrollbar(!showCustomScrollbar)}
              className="p-2 rounded-md hover:bg-muted text-muted-foreground hover:text-foreground transition-colors"
              aria-label={showCustomScrollbar ? "Hide scrollbar" : "Show scrollbar"}
            >
              {showCustomScrollbar ? <MdIcons.MdVisibility size={20} /> : <MdIcons.MdVisibilityOff size={20} />}
            </button>
            <button
              onClick={() => {
                setMessages([
        ...
        <div className="relative">
          <div
            ...
            style={{
              // Hide native scrollbar when custom scrollbar is visible
              overflowY: showCustomScrollbar ? "hidden" : "auto",
            }}
          >
            <div className="pb-4">
          ...
          {/* Custom standalone scrollbar */}
          {showCustomScrollbar && <CustomScrollbar containerRef={chatContainerRef} />}
        </div>
        ...
        <AnimatePresence>
          {showScrollButton && (
            <motion.button
              ...
              className="absolute bottom-24 right-4 p-3 rounded-full bg-primary shadow-lg text-primary-foreground hover:bg-primary/90 transition-all"
              onClick={() => {
                scrollToBottom()
                setShouldAutoScroll(true)
                setShowScrollButton(false)
          ...
          <AnimatePresence mode="wait">
            ...
            >
              {suggestions.map((suggestion, index) => (
                <SuggestionChip key={index} suggestion={suggestion} onSelect={handleSuggestionClick} />
              ))}
            </motion.div>
          </AnimatePresence>
        </div>
